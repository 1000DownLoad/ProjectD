...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler,
        IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        /// <summary>
        /// Is Initialized
        /// </summary>
        public bool IsInitialized => m_objectPool != null;
        ...
        /*******************************************************************************************************/
        /// <summary>
        /// Initalize ScrollView
        /// </summary>
        /// <param name="sampleGameObject"></param>
        /// <param name="refreshItemCallback"></param>
        /*******************************************************************************************************/
        public void Initialize(GameObject sampleGameObject, Action<Transform, int> refreshItemCallback = null, Action<Transform, int> closeItemCallback = null)
        {
            RefreshItem = refreshItemCallback;
            CloseItem = closeItemCallback;
        ...
        /*******************************************************************************************************/
        public virtual void UpdatePollTotalCount(int _totalCount)
        {
            totalCount = _totalCount;
        }

        private Action<Transform, int> RefreshItem;
        private Action<Transform, int> CloseItem;

        [HideInInspector] public int totalCount = 0;
        protected float threshold = 0;
        protected bool reverseDirection = false;
        protected float rubberScale = 1;
        ...
        protected abstract int GetConstraintCount();
        ...
        /// 스크롤 마지막 콜백.
        /// </summary>
        protected Action<bool> _endCallBack;
        protected float contentSpacing
        {
            get
            {
                ...
                if (m_Content != null)
                {
                    if (m_Content.TryGetComponent<HorizontalOrVerticalLayoutGroup>(out var layout1))
                        m_ContentSpacing = layout1.spacing;

                    if (m_Content.TryGetComponent(out m_GridLayout))
        ...
        public Scrollbar horizontalScrollbar
        {
            ...
            set
            {
                if (m_HorizontalScrollbar && null != m_HorizontalScrollbar.handleRect)
        ...
        public Scrollbar verticalScrollbar
        {
            ...
            set
            {
                if (m_VerticalScrollbar && null != m_VerticalScrollbar.handleRect)
        ...
        private bool m_useDragging = true;
        public bool useDragging { get { return m_useDragging; } set { m_useDragging = value; } }
        ...
        public void Return(GameObject select)
        {
            if (Application.isPlaying)
            {
        ...
        public void ReturnAll()
        {
            if (Application.isPlaying)
            {
                ...
                var childCount = m_Content.childCount;
        ...
        bool isUpdateScroll = true;

        public void ScrollToCell(int index, bool isUpdateScroll = true)
        {
            if (totalCount >= 0 && (index < 0 || index >= totalCount))
            {
                Debug.LogWarningFormat("invalid index {0}", index);
            ...
            this.isUpdateScroll = isUpdateScroll;
            StopAllCoroutines();
            StartCoroutine(ScrollToCellCoroutine(index));
        }

        private IEnumerator ScrollToCellCoroutine(int index)
        {
            var needMoving = true;
            while (needMoving)
            {
                ...
                float move = 1000000.0f;
                if (index >= itemTypeEnd)
                {
                    move = 1000000.0f;
                }
                else
        ...
        public void RefreshScrollRect(int offset = 0, bool isRefreshCallback = true)
        {
            /*
             * 인벤토리 슬롯 갱신이 제대로 되지 않는 버그를 발견하여, RefreshCell과 Refill을 함꼐 처리했습니다.
             * 퍼포먼스에서 문제가 발생한다면, 해당 부분을 다시 분리해서 필요한 곳에서만 사용하도록 바꿔야 할 듯.
             * (Refill의 경우, UpdateCallback을 호출하지 않고 다시 그려주기만 함)
             */
        ...
        public void RefreshCells(Transform selectCell = null)
        {
            ...
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                Transform tmpCell = null;
                for (int i = 0; i < m_Content.childCount; i++)
                {
                    tmpCell = m_Content.GetChild(i);
                    if (itemTypeEnd < totalCount)
                    {
                        if (null == selectCell || selectCell == tmpCell)
                            RefreshItem?.Invoke(tmpCell, itemTypeEnd);//dataSource.ProvideData(tmpCell, itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (null == m_objectPool) return;

            StopMovement();
            itemTypeEnd = reverseDirection ? offset : totalCount - offset;
            itemTypeStart = itemTypeEnd;

            if (totalCount >= 0 && itemTypeStart % m_ContentConstraintCount != 0)
            ...
            sizeToFill = directionSign == -1 ? viewRect.rect.size.y : viewRect.rect.size.x;
        ...
        private void RefillCells(int offset = 0)
        {
            if (!Application.isPlaying) return;
        ...
        protected float NewItemAtStart()
        {
            ...
            float size = 0;
            for (int i = 0; i < m_ContentConstraintCount; i++)
            {
                itemTypeStart--;
        ...
        /// 스크롤 마지막 콜백.
        /// </summary>
        public void EndCallBack(Action<bool> callback)
        {
        ...
        protected float NewItemAtEnd()
        {
            ...
            int count = m_ContentConstraintCount - (m_Content.childCount % m_ContentConstraintCount);
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            if (((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && itemTypeStart < m_ContentConstraintCount)
                || m_Content.childCount == 0)
            {
                return 0;
            }
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            RefreshItem?.Invoke(nextItem, itemIdx);
            return nextItem;
        }
        //==========LoopScrollRect==========
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!m_Dragging && m_useDragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
            {
                Vector2 position = m_Content.anchoredPosition;
            ...
            if (m_Dragging && m_useDragging && m_Inertia)
            {
...
public class LoopScrollRectInspector : Editor
{
    ...
    {
        if (((LoopScrollRect)target).transform.Find("ContentParent") == null)
            CreateLoopVerticalScrollRect();
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.TextField(" * 만약 내부에 Content 오브젝트가 생성되지 않으면 아래 버튼 클릭으로 생성 ");
    ...
    private GameObject CreateLoopVerticalScrollRect()
    {
        var root = ((LoopScrollRect)target).gameObject;
        var contentParent = CreateUIObject("ContentParent", root);
        var content = CreateUIObject("Content", contentParent);
        var contentRT = content.GetComponent<RectTransform>();

        root.name = "ScrollView";
        ...
        var scrollRect = ((LoopScrollRect)target);
        scrollRect.content = contentRT;
        scrollRect.viewport = contentParent.GetComponent<RectTransform>();
        ...
        if (!root.TryGetComponent<RectMask2D>(out var RectMask2D))
            RectMask2D = root.AddComponent<RectMask2D>();

        if (!content.TryGetComponent<GridLayoutGroup>(out var layoutGroup))
        ...
        if (!content.TryGetComponent<ContentSizeFitter>(out var sizeFitter))
    ...
    private static GameObject CreateUIObject(string name, GameObject parent)
    {
        ...
        if (!createdObject.TryGetComponent<RectTransform>(out var RectTransform))
            RectTransform = createdObject.AddComponent<RectTransform>();

        SetParentAndAlign(createdObject, parent);
